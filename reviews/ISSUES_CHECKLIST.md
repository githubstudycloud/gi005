...
### 版本一致性 (v3.2.2 已修复)
- [x] **VER-001**: 统一版本号 `models.py:236` 和 `app.py:122` - 已更新为 3.2.2
- [x] **VER-002**: 更新 `requirements.txt:2` 和 `Dockerfile:4` 注释中的版本号 - 已更新为 v3.2

### 配置和版本
- [ ] **DOC-004**: 统一 Python 版本约束
- [ ] **DEPLOY-006**: 锁定 requirements.txt 依赖版本
- [ ] **DEPLOY-007**: 统一 PyTorch 版本
- [x] **DEPLOY-008**: 修正 pyproject.toml 包路径 - 已更新为 v3.2.1

### 依赖一致性 (v3.2.2 已修复)
- [x] **DEP-001**: 统一 fastapi 版本 - 已统一为 >=0.104.0
- [x] **DEP-002**: 统一 numpy 版本 - 已统一为 >=1.24.0
- [ ] **DEP-003**: 创建 requirements-freeze.txt 锁定生产环境依赖
...
- [ ] **CODE-009**: 移除注释死代码
- [x] **CODE-010**: 实现 cleanup_expired() - 已实现
...
1. **VER-001** 统一代码中版本号为 3.2.2 (models.py, app.py)
2. **VER-002** 更新注释版本号为 v3.2 (requirements.txt, Dockerfile, config.yaml)
3. **DEP-001** 统一 fastapi 版本为 >=0.104.0
4. **DEP-002** 统一 numpy 版本为 >=1.24.0，scipy 版本为 >=1.10.0
5. **CFG-001** 提取 WebSocket 广播间隔到配置 (ws_broadcast_interval)
