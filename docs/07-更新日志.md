...
### 部署改进
- **Docker 版本标签**: 所有服务镜像从 `:latest` 改为 `:3.2.3` 版本标签
- **超时配置**: 提取硬编码超时值到配置文件
  - `request_timeout`: API 请求超时 (60s)
  - `batch_timeout`: 批量请求超时 (120s)
  - `health_check_timeout`: 健康检查超时 (5s)

### 代码改进
- **SystemConfig**: 新增超时配置字段
- **版本统一**: 所有组件版本号更新为 3.2.3

### 其他
- **.gitignore**: 添加 Windows 特殊文件 `nul`/`NUL` 忽略规则
...
### 版本一致性
- **版本号统一**: models.py、app.py 版本号更新为 3.2.2
- **注释版本**: requirements.txt、Dockerfile、config.yaml 注释更新为 v3.2
...
### 配置改进
- **WebSocket**: 广播间隔提取到配置 (`ws_broadcast_interval`)
...
### 部署修复
- **Dockerfile**: 改用标准安装 `pip install .` 替代 `-e` 模式（更安全）
- **docker-compose**: 修复模型路径，使用环境变量替代上级目录引用
- **docker-compose**: 修复端口冲突，CPU 版本改用 8004 端口

### 代码改进
- **models.py**: 添加 UUID 短 ID 设计说明注释
