...
**测试步骤**:
1. 执行 `python -m src.main gateway --port 8080`
2. 检查服务是否正常启动
3. 检查日志输出
...
测试过程中保存的截图:
- `test_audio/status_page.png` - 状态页面截图
...
python -m src.main worker --engine xtts --port 8001 --gateway http://localhost:8080 --device cpu --auto-load
```
...
**响应**:
```json
{"success": true, "voice_id": "test_voice", "engine": "xtts"}
```

**测试结果**: ✅ 通过
- 音色嵌入保存到 `voices/test_voice/embedding.pt` (134KB)
- 元数据保存到 `voices/test_voice/voice.json`
...
curl -X POST http://127.0.0.1:8001/synthesize \
  ...
  -d '{"text":"Hello, this is a test.","voice_id":"test_voice","language":"en"}' \
...
curl -X POST http://127.0.0.1:8001/synthesize \
  -H "Content-Type: application/json" \
  -d '{"text":"Testing the voice clone system.","voice_id":"test_voice","language":"en"}' \
...
```bash
# 停止网关进程
taskkill /F /IM python.exe /FI "WINDOWTITLE eq src*"
